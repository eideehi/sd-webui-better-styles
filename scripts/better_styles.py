  ...
  async def delete_styles(request: Request):
    ...
    for style_data in file_data:
      if (style_data.name == target_names["group"]):
        filtered_styles, removed_styles = filtering_styles(style_data.styles, lambda x: x.name in target_names["styles"])
...
def on_ui_settings():
  shared.opts.add_option("better_styles_hide_original_styles", shared.OptionInfo(False, _("Hide the original Styles"), section = SETTINGS_SECTION))
  shared.opts.add_option("better_styles_localization", shared.OptionInfo("", _("Language of Better Styles (requires reload UI)"), gr.Dropdown, lambda: {"choices": available_localization}, refresh = refresh_available_localization, section = SETTINGS_SECTION)),
