 ...
 */
export function _(text: string, ...args: Array<string | number | boolean>): string {
 ...
 */
export function withStringOption(optionName: string, callback: Callback1<string>): void {
  const value = opts[optionName];
 ...
 * @param callback - Callback function to process the retrieved option value.
 */
export function withNumberOption(optionName: string, callback: Callback1<number>): void {
  const value = opts[optionName];
